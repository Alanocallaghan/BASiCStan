...
pathfinder <- function(model, data, ...) {
    ...
    colnames(draws) <- params[!grepl("^(mu|epsilon|fu)", params)]
    .calc_generated(draws)
}

.calc_generated <- function(draws) {
  logmu <- draws[, grepl("^log_mu", colnames(draws))]
  delta <- draws[, grepl("^delta", colnames(draws))]
  mu <- exp(logmu)
  betas <- draws[, grepl("^beta", colnames(draws))]
  k <- sum(grepl("beta", colnames(draws)))
  locations <- BASiCS:::.estimateRBFLocations(colMeans(logmu), k, FALSE)
  fu <- betas %*% t(BASiCS:::.designMatrix(k, locations, colMeans(mu), 1.2))
  epsilon <- delta - fu
  colnames(epsilon) <- gsub("delta", "epsilon", colnames(epsilon))
  colnames(mu) <- gsub("log_", "", colnames(mu))
  cbind(draws, mu, epsilon)
}

pathfinder_parallel <- function(model, data, n = 4,
